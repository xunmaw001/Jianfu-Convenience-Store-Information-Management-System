	            ...
	            <div class="row">
	              ...
	              <div class="col-md-12">
	                ...
	                <div class="widget-head">
	                  <div class="pull-left">销售统计列表</div>
	                    ...
	                    <a href="#" class="wclose"><i class="icon-remove"></i></a>-->
						  <form class="navbar-form navbar-left" id="searchForm" name="searchForm" role="search" style="margin: 3px 0 3px 0; white-space: nowrap;" onsubmit="saveSearchForm()">
							  <div class="form-group" id="form-item-group">
								 																			<label>名称</label>
																					<input name="mingcheng" placeholder="名称">
																			                                 																			<label>周期</label>
																					<select name="zhouqi">
																							<option value=" "></option>
						  ...
						  <button id="addBtn" class="btn btn-success 新增" onclick="window.location.href='add-or-update.jsp'">添加</button>
						  						  <button onclick="graph()" class="btn btn-info 报表">统计</button>
	                        ...
	                        <tr>
								<th><input type="checkbox" onclick="chooseAll()"/></th>
																											<th onclick="sort('mingcheng')">名称<i id="mingchengIcon" class="fa fa-sort"></i></th>
																																				<th onclick="sort('zhouqi')">周期<i id="zhouqiIcon" class="fa fa-sort"></i></th>
																																				<th onclick="sort('yingyezonge')">营业总额<i id="yingyezongeIcon" class="fa fa-sort"></i></th>
	...
	<span class="totop"><a href="#"><i class="icon-chevron-up"></i></a></span> 
		...
		function setDataRow(item){
			...
			row.appendChild(chkTd);
            				...
            				var zhouqiCell  = document.createElement('td');

												  zhouqiCell.innerHTML = item.zhouqi; 
				  
                
									row.appendChild(zhouqiCell);
				
            				var yingyezongeCell  = document.createElement('td');

												 yingyezongeCell.innerHTML = item.yingyezonge;

                
									row.appendChild(yingyezongeCell);
		...
		function setSearchForm() {
			//设置查询表单的url
			var url = baseUrl + 'xiaoshoutongji/page';
		...
		function saveSearchForm() {
			searchForm = { key: ""};
			pageIndex = 1;
																		var mingcheng = document.forms["searchForm"]["mingcheng"].value;
						if(mingcheng != null && mingcheng.trim() != ""){
							searchForm.mingcheng = "%" + mingcheng + "%"
						}
									 
																						var zhouqi = document.forms["searchForm"]["zhouqi"].value;
						if(zhouqi != null && zhouqi.trim() != ""){
							searchForm.zhouqi = "%" + zhouqi + "%"
						}
		...
		function datePick(){
			$.fn.datetimepicker.dates['zh-CN'] = { 
		            ...
		            meridiem: ["上午", "下午"]
																																	...
																																	$(function() { 
						$('#dengjiriqi_start').datetimepicker({ 
						pickTime: false,
						language:'zh-CN'
						});
					});
					$(function() { 
						$('#dengjiriqi_end').datetimepicker({ 
						pickTime: false,
						language:'zh-CN'
						});
					});
