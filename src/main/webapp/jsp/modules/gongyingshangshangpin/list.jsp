  ...
  <body>
		...
		<%@ include file="../../static/sidebar.jsp"%>
	      ...
	      <div class="page-head">
	        ...
	        <div class="bread-crumb pull-right">
	          ...
	          <a href="#" class="bread-current">供应商商品</a>
	        </div>
	            ...
	            <div class="row">
	              ...
	              <div class="col-md-12">
	                ...
	                <div class="widget-head">
	                  <div class="pull-left">供应商商品列表</div>
	                    ...
	                    <a href="#" class="wclose"><i class="icon-remove"></i></a>-->
						  <form class="navbar-form navbar-left" id="searchForm" name="searchForm" role="search" style="margin: 3px 0 3px 0; white-space: nowrap;" onsubmit="saveSearchForm()">
							  <div class="form-group" id="form-item-group">
								 																			<label>商品编号</label>
																					<input name="shangpinbianhao" placeholder="商品编号">
																			                                 																			<label>商品名称</label>
																					<input name="shangpinmingcheng" placeholder="商品名称">
																			                                 																			<label>商品类型</label>
																					<select name="shangpinleixing">
																						</select>
	                        ...
	                        <tr>
								<th><input type="checkbox" onclick="chooseAll()"/></th>
																											<th onclick="sort('shangpinbianhao')">商品编号<i id="shangpinbianhaoIcon" class="fa fa-sort"></i></th>
																																				...
																																				<th onclick="sort('yuexiaoliang')">月销量<i id="yuexiaoliangIcon" class="fa fa-sort"></i></th>
	...
	<span class="totop"><a href="#"><i class="icon-chevron-up"></i></a></span> 
		...
		function setDataRow(item){
			...
			row.appendChild(chkTd);
            				...
            				var tupianCell  = document.createElement('td');
								                  ...
								                  var tupianCellImg = document.createElement('img');
				  tupianCellImg.setAttribute('src',item.tupian);
				  tupianCellImg.setAttribute('height',100);
				  tupianCellImg.setAttribute('width',100);
				  tupianCell.appendChild(tupianCellImg);
            				...
            				var jianshuCell  = document.createElement('td');
												  ...
												  //如果图文详情包含图片
				  if(item.jianshu && item.jianshu.indexOf('img') != -1){
					//暂时只考虑图片+文字 和 文字+图片的情况
					var beginIndex = item.jianshu.indexOf('<img>');
					var endIndex = item.jianshu.indexOf('>');				
					jianshuCell.innerHTML = item.jianshu.substring(beginIndex, endIndex+1).replace("img","img width='100' height='100'");				
				  }else{
				  	 if(item.jianshu != null && item.jianshu != "" && item.jianshu.length >= 11){
				  		jianshuCell.innerHTML = item.jianshu.substring(0, 10) + "..."; 
				  	}else{
				  		jianshuCell.innerHTML = item.jianshu;
            				...
            				var yuexiaoliangCell  = document.createElement('td');

												 yuexiaoliangCell.innerHTML = item.yuexiaoliang;

                
									row.appendChild(yuexiaoliangCell);
			...
			crossBtn1.setAttribute("class","btn btn-xs btn-primary");
			crossBtn1.setAttribute("onclick","crossTable(" + item.id + ",\"jinhuoxinxi\")");
		...
		function setSearchForm() {
			//设置查询表单的url
			var url = baseUrl + 'gongyingshangshangpin/page';
		...
		function saveSearchForm() {
			searchForm = { key: ""};
			pageIndex = 1;
																		var shangpinbianhao = document.forms["searchForm"]["shangpinbianhao"].value;
						if(shangpinbianhao != null && shangpinbianhao.trim() != ""){
							searchForm.shangpinbianhao = "%" + shangpinbianhao + "%"
						}
									 
																						var shangpinmingcheng = document.forms["searchForm"]["shangpinmingcheng"].value;
						if(shangpinmingcheng != null && shangpinmingcheng.trim() != ""){
							searchForm.shangpinmingcheng = "%" + shangpinmingcheng + "%"
						}
									 
																						var shangpinleixing = document.forms["searchForm"]["shangpinleixing"].value;
						if(shangpinleixing != null && shangpinleixing.trim() != ""){
							searchForm.shangpinleixing = "%" + shangpinleixing + "%"
						}
		...
		function datePick(){
			$.fn.datetimepicker.dates['zh-CN'] = { 
		            ...
		            meridiem: ["上午", "下午"]
																																																																				...
																																																																				$(function() { 
						$('#shangjiariqi_start').datetimepicker({ 
						pickTime: false,
						language:'zh-CN'
						});
					});
					$(function() { 
						$('#shangjiariqi_end').datetimepicker({ 
						pickTime: false,
						language:'zh-CN'
						});
					});
