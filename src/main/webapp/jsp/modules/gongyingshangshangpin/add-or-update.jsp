						...
						<form class="form-horizontal" id="addOrUpdateForm" method="post">
							<input id="updateId" name="id" type="hidden">
										...
										<div class="col-lg-6">
											...
											<div class="form-group">
										<label class="control-label col-lg-3">月销量</label>
										<div class="col-lg-6">
											<input id="yuexiaoliang" name="yuexiaoliang" placeholder="月销量">
										</div>
									</div>	
											<div class="form-group">
										...
										<div class="col-lg-6">
											<input id="gongsimingcheng" name="gongsimingcheng" placeholder="公司名称">
										</div>
									</div>	
	
																																																																																																																																																	<div class="form-group">
											<label class="control-label col-lg-3">简述</label>
											<div class="col-lg-6">
												<input id="jianshuupload" name="file" type="file">
												<script id="jianshuEditor" type="text/plain" style="width:800px;height:230px;"></script>
												<script type="text/javascript">
												    //建议使用工厂方法getEditor创建和引用编辑器实例，如果在某个闭包下引用该编辑器，直接调用UE.getEditor('editor')就能拿到相关的实例
												   //相见文档配置属于你自己的编译器
												 var jianshuUe = UE.getEditor('jianshuEditor', {
													toolbars: [
														[
															'undo', //撤销
															'bold', //加粗
												...
												<input type="hidden" name="jianshu" id="jianshu-input">
											</div>
										</div>
																																																																																																																																																																																																																															<!-- 表单按钮 -->
							<div class="form-group">
	...
	<script type="text/javascript">
		...
		function upload(){
						...
						dataType: 'json',
							...
							$('#jianshuupload').fileupload({
						url: baseUrl + 'file/upload',
						headers:{ token: window.sessionStorage.getItem("token")},
						dataType: 'json',
						done: function (e, data) {	
							UE.getEditor('jianshuEditor').execCommand('insertHtml','<img src=\"'+ baseUrl +'upload/'+ data.result.file + '\" width=900 height=560 >');					
						}
					});
									}
		// 表单提交
		...
		// 填充富文本框
		function setContent(){
																																		if(ruleForm.jianshu != null && ruleForm.jianshu != 'null' && ruleForm.jianshu != ''){
						var jianshuMes = '' + ruleForm.jianshu;
						var jianshuUeditor = UE.getEditor('jianshuEditor');
						jianshuUeditor.ready(function() {
							jianshuUeditor.setContent(jianshuMes);
						});
				}	
																																												}  
		// 获取富文本框内容
		function getContent(){
																																		if(UE.getEditor('jianshuEditor').hasContents()){
					$('#jianshu-input').attr('value',UE.getEditor('jianshuEditor').getPlainTxt());
				}	
																																												}
					...
					if(res.code == 0){
						var myId = res.data.id;
							...
							$('#zhanghao').val(res.data.zhanghao)
								...
								if(res.data !=null && res.data.gongsimingcheng != null && res.data.gongsimingcheng != ''){
					...
					for(var shangpinleixingCount = 0; shangpinleixingCount < shangpinleixingOptions.length;shangpinleixingCount++){
						if(shangpinleixingOptions[shangpinleixingCount].getAttribute('value') == ruleForm.shangpinleixing){
							shangpinleixingOptions[shangpinleixingCount].setAttribute('selected','selected');
																...
																$("#yuexiaoliang").val(ruleForm.yuexiaoliang);	
					...
					if(res.code == 0){
						...
						$('#updateId').val(crossTableId);
								...
								$("#tupian-input").val(res.data.tupian);
												...
												if(res.data != null && res.data != '' && res.data.yuexiaoliang != null && res.data.yuexiaoliang != ''){

															$("#yuexiaoliang").val(res.data.yuexiaoliang);
													}
