	            ...
	            <div class="row">
	              ...
	              <div class="col-md-12">
	                ...
	                <div class="widget-head">
	                  <div class="pull-left">进货信息列表</div>
	                    ...
	                    <a href="#" class="wclose"><i class="icon-remove"></i></a>-->
						  <form class="navbar-form navbar-left" id="searchForm" name="searchForm" role="search" style="margin: 3px 0 3px 0; white-space: nowrap;" onsubmit="saveSearchForm()">
							  <div class="form-group" id="form-item-group">
								 																			<label>订单编号</label>
																					<input name="dingdanbianhao" placeholder="订单编号">
																			                                 																			<label>商品名称</label>
																					<input name="shangpinmingcheng" placeholder="商品名称">
																			                                 																			<label>商品类型</label>
																					<input name="shangpinleixing" placeholder="商品类型">
																			                                 									                                 									                                 									                                 									                                 									                                 									                                 									                                 							  </div>
						  </form>
	...
	<span class="totop"><a href="#"><i class="icon-chevron-up"></i></a></span> 
		...
		function setDataRow(item){
			...
			row.appendChild(chkTd);
					...
					payBtn.innerHTML = "支付";           
					payBtn.setAttribute('class','btn btn-xs btn-success 支付')
        ...
        function remove(id) { 
            ...
            if (mymessage == true) {
				...
				httpJson("jinhuoxinxi/delete","POST",paramArray,(res)=>{
					if(res.code == 0){
						getDataList();		
						alert('删除成功');
					}
		...
		function setSearchForm() {
			//设置查询表单的url
			var url = baseUrl + 'jinhuoxinxi/page';
		...
		function saveSearchForm() {
			searchForm = { key: ""};
			pageIndex = 1;
																		var dingdanbianhao = document.forms["searchForm"]["dingdanbianhao"].value;
						if(dingdanbianhao != null && dingdanbianhao.trim() != ""){
							searchForm.dingdanbianhao = "%" + dingdanbianhao + "%"
						}
		...
		function datePick(){
			$.fn.datetimepicker.dates['zh-CN'] = { 
		            ...
		            meridiem: ["上午", "下午"]
																																																													...
																																																													$(function() { 
						$('#jinhuoriqi_start').datetimepicker({ 
						pickTime: false,
						language:'zh-CN'
						});
					});
					$(function() { 
						$('#jinhuoriqi_end').datetimepicker({ 
						pickTime: false,
						language:'zh-CN'
						});
					});
		...
		function getSum(colName){
			...
			window.sessionStorage.setItem('crossTableName',"jinhuoxinxi");
        ...
        function pay(id){
            window.sessionStorage.setItem('id',id);
			window.sessionStorage.setItem('tableName',"jinhuoxinxi");
